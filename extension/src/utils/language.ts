...
export const lastAvailableLanguages = async () => {
  const cacheLanguages = (await loadFromCache(languagesUrl)) as
    | IfetchResponse<Ilanguages>
    | undefined;
  if (cacheLanguages) {
    return {
...
const getSupportedSystemLanguage = async () => {
  let availableLanguagesStorage = await browser.storage.local.get([
...
export const setLanguage = async (lang: string) => {
  const previousLang = (
    await browser.storage.sync.get([langKey]).catch(() => {})
  )?.[langKey];
