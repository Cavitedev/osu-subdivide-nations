...
export const idFromOsuProfileUrl = (url: string | null | undefined): string | null => {
    if(!url) return null;
    if(!url.includes("//osu.ppy.sh/") && !url.includes("//old.ppy.sh/")) return null;
    if(!url.includes("ppy.sh/users/") && !url.includes("ppy.sh/u/")) return null;
