import {IregionData, IosuWorldRegionalPlayerData } from './../utils/tools';
...
(async () => {
  ...
  const getRegionNames = async (countryCode: string) => {
    ...
    if ("lang" in regionsOsuWorld && regionsOsuWorld["lang"] === tools.nativeLanguageCode) {
  ...
  const updateFlagUser = async (item: HTMLElement, userId: string, addMargin = true) => {
    if (!item) return;
    const playerOsuWorld = await tools.osuWorldUser(userId);
    if (playerOsuWorld.error) {
      if (playerOsuWorld.error.code === tools.unknownUserError) {
        return;
      }
  ...
  const updateRankingRow = async (row:HTMLElement, playerData: IosuWorldRegionalPlayerData) => {
    ...
    nameElement.setAttribute("href", tools.buildProfileUrl(id.toString(), mode));
  ...
  const updatePagePagination = (paginationItem:HTMLElement, page: number) => {
    ...
    if (href) {
      link.setAttribute(
        "href",
        tools.addOrReplaceQueryParam(href, "page", page.toString())
      );
    }
  };
